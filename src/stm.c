...
_CALLCONV stm_tx_t * 
stm_pos_init_thread(char *name){ 
  return int_stm_pos_init_thread(name) ; 	
} 
_CALLCONV void
stm_pos_exit_thread(char *name){ 
  TX_GET; 
  int_stm_pos_exit_thread(name,tx) ;
} 
#endif 
/*
...
void delayed_cache_flush(){ 
	TX_GET ; 	
	int i = 0 ; 	
	for( i = 0 ; i < tx->delayed_index ; i++){ 
		cf_count++ ; 	
		void * vend = tx->delayed_addr[i]+tx->delayed_size[i]-1;
//		printf("start : %p\n , end : %p\n" , tx->delayed_addr[i] , vend) ; 	
		mb() ; 	
		for( ; tx->delayed_addr[i] < vend ; 
			tx->delayed_addr[i]+=x86_64_clflush_size)
			clflush(tx->delayed_addr[i]);
		clflush(vend) ; 
		mb() ; 	
	}
	// initialize delayed index because started new transaction.
	tx->delayed_index = 0 ; 	

}  
void delayed_cache_buffer( void * addr ,unsigned long size ){ 
	TX_GET ;
//	printf("[%s]\n" , __func__ ) ; 	
//	printf("index : %d\n" , tx->delayed_index) ; 
	tx->delayed_addr[tx->delayed_index] = addr ; 	
//	printf("%p\n" , tx->delayed_addr[tx->delayed_index]); 	
	tx->delayed_size[tx->delayed_index] = size ; 	
//	printf("%d\n" , tx->delayed_size[tx->delayed_index]) ; 

	tx->delayed_index++ ; 	
	return ; 	
} 
void print_delayed_buffer(){ 
	TX_GET ; 	
	int i = 0 ; 	
	for( i = 0 ; i < tx->delayed_index ; i++ ){ 
		printf("%p\n" , tx->delayed_addr[i]) ; 	
