...
static NOINLINE void 
stm_pos_allocate_rs_entries(stm_tx_t *tx, int extend){ 
  if (extend) {
    /* Extend read set */
    tx->r_set.size *= 2;
    tx->r_set.entries = (r_entry_t *)xrealloc(tx->r_set.entries, tx->r_set.size * sizeof(r_entry_t));
  } else {
      tx->r_set.entries = (r_entry_t *)rs_ws_malloc_aligned( obj_store , tx->r_set.size * sizeof(r_entry_t));
  }
} 
/*
...
static NOINLINE void
stm_pos_allocate_ws_entries(stm_tx_t *tx, int extend)
{
#if CM == CM_MODULAR || defined(CONFLICT_TRACKING)
  int i, first = (extend ? tx->w_set.size : 0);
#endif /* CM == CM_MODULAR || defined(CONFLICT_TRACKING) */
  ...
  if (extend) {
    ...
    a = tx->w_set.entries;
    /* pos_malloc */ 	
    tx->w_set.entries = (w_entry_t *)rs_ws_malloc_aligned(obj_store, tx->w_set.size*sizeof(w_entry_t)); 
...
#define tiny_mb() asm volatile("mfence":::"memory")
void tiny_clflush( volatile void *__p ){ 
...
#define SUB_TM 2 
#define NAME_TM "_TM"
/* TM_POS_INIT_THREAD(name) called */ 

static INLINE stm_tx_t *
int_stm_pos_init_thread( char *name)
{
  ...
  /* At first create object storage matching with thread_id */ 	
  strcpy( log_name , name ) ; 	
  strcat( log_name , NAME_TM) ; 
  /* when routine finish to make OBJ_TM */ 
  ...
  printf("[%s][log_name : %s]\n" , __func__ , log_name) ; 	
  #endif 
  /* cancel to make this storage object */ 
  //ret_val = stm_pos_create_object( log_name , SUB_TM ) ; 	 

#ifdef EPOCH_GC
  gc_init_thread();
#endif /* EPOCH_GC */

  /* Allocate descriptor in nvram */
  tx = tx_malloc_aligned( log_name , sizeof(stm_tx_t)) ; 
  if( tx == NULL ){ 
	printf("Failed to allocate tx desc in nvram\n") ; 	
	exit(-1) ; 	
  } 
  /* Set attribute */
  ...
  printf("before readset\n") ; 	
  stm_pos_allocate_rs_entries(tx,0) ;
  ...
  tx->w_set.bloom = 0; 
#endif /* USE_BLOOM_FILTER */
  printf("before writeset\n") ;
  stm_pos_allocate_ws_entries(tx, 0) ;
  //stm_allocate_ws_entries(tx, 0);
  /* Nesting level */
  tx->nesting = 0;
...
static INLINE void
int_stm_pos_exit_thread(char *name,stm_tx_t *tx){ 
